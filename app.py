...
# from gemini_sdk import GeminiAPI  # Assuming gemini_sdk is the package name
...
def download_youtube_audio(video_url):
    try:
        ...
        audio_file_path = os.path.join(download_dir, f"{sanitized_title}.mp4")
        audio_stream.download(download_dir, filename=f"{sanitized_title}.mp4")

        # Convert .mp4 audio to .mp3
        audio = AudioSegment.from_file(audio_file_path)
        mp3_file_path = os.path.join(download_dir, f"{sanitized_title}.mp3")
        audio.export(mp3_file_path, format="mp3")

        # Remove the .mp4 audio file
        os.remove(audio_file_path)

        st.success(f"Audio downloaded successfully at: {mp3_file_path}")
...
def get_text_as_pdf(video_url):
    try:
        ...
        download_dir = os.path.join(os.path.expanduser("~"), "Downloads")
        sanitized_title = sanitize_filename(yt.title)
        temp_audio_path = os.path.join(download_dir, f"{sanitized_title}.mp4")
        audio_stream.download(download_dir, filename=f"{sanitized_title}.mp4")
...
def get_vector(chunks, index_name="langchain-chatbot"):
...
def get_conversation_chain(vectorstore):
    ...
    # The line `model = ChatAnthropic(model='claude-3-opus-20240229',
    # api_key=os.getenv("ANTHROPIC_API_KEY"))` is creating an instance of the `ChatAnthropic` class
    # with specific parameters. Here's what each part of the line is doing:
